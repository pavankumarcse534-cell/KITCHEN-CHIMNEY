...
def main():
    ...
    for model_type in model_types:
        ...
        if not design:
            ...
            design.save()
            print(f"   ✅ Design is active")
        
        # Migrate legacy GLB file if exists
        if design.model_file:
            print(f"   📄 Found legacy model_file, migrating...")
            ...
            if glb_count > 0:
                print(f"   ✅ {glb_count} GLB file(s) already exist:")
                for glb_file in glb_files:
                    print(f"      - {glb_file.file_name or glb_file.file.name} (ID: {glb_file.id}, Primary: {glb_file.is_primary})")
                success_count += 1
            else:
    ...
    print("   - Both model types exist and are active")
    print("   - Ready for frontend use")
    print()
    print("💡 Frontend Status:")
    print("   - Combined option available in dropdown")
    print("   - Will load GLB files from both model types")
    print()
    print("💡 Next Steps:")
    print("   1. Check frontend: http://localhost:3000/")
    print("   2. Select combined model type from dropdown")
    print("   3. Both GLB files will load if available")
    print("="*60)
